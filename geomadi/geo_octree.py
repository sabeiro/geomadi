...
class h3tree(quadtree):
    """index based on uber-h3 geohash"""
    def __init__(self,BoundBox=[5.866,47.2704,15.0377,55.0574],max_precision=20,padding=0.1):
        from h3 import h3

    def encode(self,x,y,precision=8):
        """encode a geohash calling the original library"""
        return h3.geo_to_h3(x,y,precision)
    ...
    def decodePoly(self,sId):
        """decode a geohash calling the original library"""
        return h3.h3_to_geo_boundary(sId, geo_json=False)
...
def densGroup(dens,max_iter=5,threshold=30,geo_col="octree"):
    """removoe a digit from octree and group the coarse octree together until the threshold density is met"""
    dens.loc[:,geo_col] = dens[geo_col].astype(str)
    dens = dens.groupby(geo_col).agg(np.sum).reset_index()
    for i in range(max_iter):
        print("grouping iter %d entries %d" % (i,dens.shape[0]))
        setL = dens['n'] < threshold
        if sum(setL) == 0: break
        dens.loc[setL,geo_col] = dens.loc[setL,geo_col].apply(lambda x: x[:-1])
        dens = dens.groupby(geo_col).agg(np.sum).reset_index()
    dens = dens[dens[geo_col] != '']
    dens.loc[:,geo_col] = dens[geo_col]#.astype(int)
    return dens

def densGroupAv(dens,max_iter=5,threshold=30,geo_col="octree"):
    """remove a digit from octree and group the coarse octree together until the threshold density is met"""
    dens.loc[:,geo_col] = dens[geo_col].astype(str)
    dens = dens.groupby(geo_col).agg(np.sum).reset_index()
    tL = [x for x in dens if not x in [geo_col,'n']]
    for i in range(max_iter):
        print("grouping iter %d entries %d" % (i,dens.shape[0]))
        setL = dens['n'] < threshold
        if sum(setL) == 0: break
    ...
    if False:
        ...
        wm.__name__ = 'wa'
        g = dens.groupby(geo_col).agg({'speed':wm,'n':'sum'})
