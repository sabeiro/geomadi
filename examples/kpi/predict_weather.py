...
print('----------------------------load----------------------------------')
weatL = pd.read_csv(baseDir + "raw/"+custD+"/weather_h.csv.gz",compression="gzip",dtype={idField:str})
tL = ['cloudCover','dewPoint','humidity','precipIntensity','precipProbability','temperature','visibility','windSpeed']
for i in tL: weatL.loc[:,i] = s_s.interpMissing(weatL.loc[:,i])
for i in tL: weatL.loc[:,i] = s_s.serRunAv(weatL.loc[:,i],steps=5)

import importlib
importlib.reload(t_r)
dlX = t_r.splitInWeek(weatL,isBackfold=False,col="temperature",isInterp=True,roll=-2)
dlY = t_r.splitInWeek(weatL,isBackfold=False,col='humidity',isInterp=True,roll=-2)
print(len(dlX))
...
tK = t_k.weekImg(XL,model_type=version)
if True:
    tK.plotImg(nline=8)
