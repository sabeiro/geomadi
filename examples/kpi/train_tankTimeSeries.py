...
#    bast = pd.concat([bast,pd.read_csv(baseDir + "log/bast/bast17.csv.gz",compression="gzip")],axis=0)
    ...
    mist = pd.read_csv(baseDir + "raw/tank/ref_vist_h.csv.gz",compression="gzip")
...
X, idL, den ,norm = t_k.splitInWeek(sact,idField="id_poi")
...
tK.simpleEncoder(epoch=150)
tK.deepEncoder(epoch=50)
#tK.convNet(epoch=50)
