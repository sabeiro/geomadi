...
tK.runEncoder(YL,epochs=300)

tK.setOptimizer(name=optL[1],lr=0.01,decay=1e-6,verbose=0,split_set=1.)
tK.setX(ZL)
tK.runAutoencoder(epochs=100)
...
if False:
    ...
    t_v.showPredicted(dlX[i],encoder,isPlot=True)
...
if False:
    corL.to_csv(baseDir + "raw/"+custD+"/scor/scor_enc_"+version+".csv",index=False)
    corX.to_csv(baseDir + "raw/"+custD+"/scor/scor_enc_ext_"+version+".csv",index=False)
    tK.saveModel(baseDir + "train/"+custD+"/enc_"+version)
...
if False:
    ...
    corX = pd.read_csv(baseDir + "raw/"+custD+"/scor/scor_enc_ext_"+version+".csv")
