...
junG = gpd.GeoDataFrame.from_file(baseDir + "gis/nissan/de_junct.shp")
nodJ = gpd.GeoDataFrame.from_file(baseDir + "gis/nissan/motorway_link_nodes.shp")
...
if False:
    ...
    junG.to_file(baseDir + "gis/nissan/de_junct.shp")
...
if True:
    ...
    junT = gpd.GeoDataFrame.from_file(baseDir + "gis/nissan/de_junct_unique.shp")
    junT

    junT.to_file(baseDir + "gis/nissan/de_junct_unique.shp")
...
if False:
    ...
    nodS.to_csv(baseDir + "raw/nissan/motorway_link_nodes.csv.gz",compression="gzip",index=False)

if False:
    plog('-------------prepare-log-file----------------')
#    junT = gpd.read_file(baseDir + 'gis/nissan/de_junct_unique.shp')
    junT = gpd.read_file(baseDir + 'gis/nissan/motorway_link_nodes.shp')
    ...
    junT.to_csv(baseDir + "raw/nissan/nodes_job.csv",index=False)
    
    
    #    junT = junT.loc[junT['GEN'].isin(['Thüringen'])]
    JunL = np.unique(junT['id_jun'])
    nodeL = pd.read_csv(baseDir + "raw/nissan/node_link_add.csv.gz",compression="gzip")
...
if False:
    plog('--------------------check-nissan-nodes-in-graph---------------------')
    ...
    nodV.to_csv(baseDir + "raw/nissan/nodes_job.csv",index=False)
