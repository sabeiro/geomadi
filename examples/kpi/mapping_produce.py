...
mapL = pd.read_csv(baseDir + "raw/"+custD+"/map_cilac.csv.gz",compression="gzip",dtype={idField:str})
mapL.loc[:,"weight"] = 0.
...
lact = []
for f in fL:
    fName = baseDir + "raw/"+custD+"/act_cilac_"+version+"/"+f
    sact, hL = t_e.loadDf(fName,dateL,poi,mapL,isChi=False,idField="id_poi",hL1=hL1)
    lact.append(sact)
if len(fL) > 1:
    sact = pd.merge(lact[0],lact[1],on='cilac',how="outer",suffixes=["_x",""])
if len(fL) > 2:
    sact = sact.merge(lact[2],on='cilac',how="outer",suffixes=["_x",""])
for i in [x for x in sact.columns if bool(re.search("_x",x))]:
    try: del sact[i]
    except: print(i)
...
#tMod = tlib.regName(XL,yL,modName="elastic")
print(hL)
sact, weiL, pact = t_e.cilacLinWeight(sact,mist,hL,idField)

print('---------------------------------collect--------------------------')
weiL1 = sact[[idField,'cilac','weight']]
tact = sact.copy()
...
weiL.to_csv(baseDir+"raw/"+custD+"/map_v"+version+".csv.gz",compression="gzip",index=False)
tact.to_csv(baseDir+"raw/"+custD+"/act_weighted/act_weighted_v"+version+".csv.gz",compression="gzip",index=False)
print("unique poi %d " % len(set(tact[idField])))

if True:
    gact = sact.copy()
    gact.loc[:,"weight"] = pd.merge(gact,weiL,on=[idField,"cilac"],how="left")['weight_y']
    gact.loc[:,hL] = np.multiply(gact[hL].values,gact['weight'].values[:,np.newaxis])
    gact = gact[[idField] + hL].groupby(idField).agg(np.sum).reset_index()
    i = '1001'
    p1 = tact[tact[idField] == i]
    p2 = gact[gact[idField] == i]
    p3 = mist.loc[mist[idField] == i]
    p3 = p3[p3.columns[p3.columns >= '2019-01-01']]
    p4 = pact.loc[pact[idField] == i]
    for y in [p1,p2,p3]:
        del y[idField]
    t1 = t_r.day2time(t_r.timeCol(p1))
    t2 = t_r.day2time(t_r.timeCol(p2))
    t3 = t_r.day2time(t_r.timeCol(p3))
    t4 = t_r.day2time(p4['day'])
    plt.title("id_poi %s " % (i) )
    plt.plot(t1,p1.values[0],label="extension")
    plt.plot(t3,p3.values[0],label="reference")
    plt.plot(t2,p2.values[0],"-.",label="merge")
    plt.plot(t4,p4['value'].values,"-.",label="pact")
    plt.legend()
...
if ops['isScore']:
    ...
    scor.append(scorM1)
    scor.append(scorM2)
    scorD.append({"idx":"cell","raw":np.mean(scorM1["r_raw"]),"map":np.mean(scorM2["r_map"])})
...
if False:
    ...
    i = [y for y,x in enumerate(idL) if x == '1218'][0]
    X, y = XL[i], yL[i]
    tml = modL.modelList()
    clf = tml.regL['elastic']['mod']
    fit_w = clf.fit(X,y)
    clf = tml.regL['elastic_cv']['mod']
    fit_c = clf.fit(X,y)
    y1 = fit_w.predict(X)
    y2 = fit_c.predict(X)
    r1, r2 = sp.stats.pearsonr(y,y1)[0],sp.stats.pearsonr(y,y2)[0]
    plt.title("poi %d cor %.2f %.2f" % (i,r1,r2))
    plt.plot(y1,label="elastic")
    plt.plot(y,label="ref")
    plt.plot(y2,label="elastic_cv")
    plt.legend()
    plt.show()
