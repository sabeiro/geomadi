...
opsS = json.load(open(baseDir + "src/conf/tank_ops.json"))['smooth_hourly']
ops = opsS['start']
for i in opsS['play_blind'].keys():
    ops[i] = opsS['play_blind'][i]
...
if ops['f_etl']:  # etl_tankYear.py
    mist = pd.read_csv(baseDir + "raw/"+custD+"/act_vist_year.csv.gz",compression="gzip") 
    ...
    mist = pd.read_csv(baseDir + "raw/"+custD+"/act_gauss_year.csv.gz",compression="gzip")
    mist.loc[:,"time"] = mist[['day','hour']].apply(lambda x: "%sT%02d:00:00" %(x[0],int(x[1])),axis=1)
elif ops['f_delivery']:
    mist = pd.read_csv(baseDir + "raw/"+custD+"/delivery/act_vist_final.csv.zip",compression="zip")
    mist.loc[:,"day"]  = mist['time'].apply(lambda x:x[0:10])
    mist.loc[:,"hour"] = mist['time'].apply(lambda x:x[11:13])
    mist = pd.merge(mist,poi[['id_poi',idField]],on="id_poi",how="left")
elif ops['f_play']: # etl_tankTimeSeries.py
    mist = pd.read_csv(baseDir + "raw/"+custD+"/act_vist_play.csv.gz",compression="gzip")
    mist.loc[:,"time"] = mist[['day','hour']].apply(lambda x: "%sT%02d:00:00" %(x[0],int(x[1])),axis=1)
else:  # regression_tankYear.py
    mist = pd.read_csv(baseDir + "raw/"+custD+"/act_vist_reg_year.csv.gz",compression="gzip")
    mist.loc[:,"time"] = mist[['day','hour']].apply(lambda x: "%sT%02d:00:00" %(x[0],int(x[1])),axis=1)
...
map10 = e_m.map2df(json.load(open(baseDir + "/raw/"+custD+"/tank_und_rast_"+str(dTh)+".json")))
...
gvist = mist[[idField,'act']].groupby(idField).agg(len).reset_index()
poi.loc[:,"c_source"] = pd.merge(poi,gvist,on=idField,how="left")['act']
...
if ops['rem_anomaly']:
    poiAr = pd.read_csv(baseDir + "raw/"+custD+"/poi_anomaly_ref.csv")
    poiAa = pd.read_csv(baseDir + "raw/"+custD+"/poi_anomaly_act.csv")
    mist = mist[~mist[idField].isin(poiAr['id'])]
    mist = mist[~mist[idField].isin(poiAa['id'])]
...
if ops['weekday_correction']:
    ...
    gist = mist1[[idField,'wday','y_dif']].groupby([idField,'wday']).apply(clampF).reset_index()
    #gist = pd.read_csv(baseDir + "raw/"+custD+"/weekday_act_correction.csv")
    del mist['y_dif']
    mist = pd.merge(mist,gist,on=[idField,"wday"],how="left")
    mist.replace(np.nan,1.,inplace=True)
    mist.loc[:,"act"] = mist['act_raw']*mist['y_dif']
else:
    mist.loc[:,"act_raw"] = mist['act']

print("total locations %d:" % (len(set(mist[idField]))) )
isSingle = False
mistW = mist[mist['wday'] < 4]
if ops['hour_smoothing']:
    plog("applying smoothing")
    for g in [mist,mistW]:
        g.loc[:,"act"] = l_c.smoothClust(g['act'],g[idField],width=5,steps=9,isSingle=ops['isSingle'])
        g.loc[:,"ref"] = l_c.smoothClust(g['ref'],g[idField],width=5,steps=7,isSingle=ops['isSingle'])
if ops['hour_regressor']:
    plog("applying regressor")
    for g in [mist,mistW]:
        g.loc[:,"act"] = g['act_raw']*l_c.regressorTank(g,poi,idFloat="act")

dist  = mist[ [idField,'day','ref','act','act_raw']].groupby([idField,'day']).agg(sum).reset_index()
distW = mistW[[idField,'day','ref','act','act_raw']].groupby([idField,'day']).agg(sum).reset_index()
...
if ops['day_smoothing']:
    plog("applying smoothing")
    for g in [dist,distW]:
        g.loc[:,"act"] = l_c.smoothClust(g['act'],g[idField],width=3,steps=5,isSingle=ops['isSingle'])
        g.loc[:,"act"] = l_c.smoothClust(g['act'],g[idField],width=3,steps=5,isSingle=ops['isSingle'])
        g.loc[:,"ref"] = l_c.smoothClust(g['ref'],g[idField],width=3,steps=5,isSingle=ops['isSingle'])
...
scorHL = mist.groupby( idField).apply(clampF).reset_index()
scorHW = mistW.groupby(idField).apply(clampF).reset_index()
scorDL = dist.groupby( idField).apply(clampF).reset_index()
scorDW = distW.groupby(idField).apply(clampF).reset_index()
scorHA = mist.groupby( 'day').apply(clampF).reset_index()

cist = mist.groupby(idField).agg(len).reset_index()
cist.sort_values("time",inplace=True)

for g in [scorHL,scorHW,scorDL,scorDW]:
    g.loc[:,"d_20"] = (g['s_20'] - g['s_ref'])/g['s_ref']
    g.loc[:,"n_h"] = pd.merge(g,cist,on=idField,how="left")['time']
    g.sort_values('r_20',inplace=True)

scor = pd.DataFrame({idField:scorHL[idField]
...
scor = pd.merge(scor,poi[[idField,'x','y','daily_visit',"id_poi"]],on=idField,how="left")

if ops['f_etl']:  # etl_tankYear.py
    scor.to_csv(baseDir + "raw/"+custD+"/scor_raw.csv",index=False)
elif ops['f_timeSeries']: # etl_tankTimeSeries.py
    scor.to_csv(baseDir + "raw/"+custD+"/scor_reg.csv",index=False)
elif ops['f_delivery']:
    scor.to_csv(baseDir + "raw/"+custD+"/scor_delivery.csv",index=False)
elif ops['f_play']:
    scor.to_csv(baseDir + "raw/"+custD+"/scor_play_smooth.csv",index=False)
else:  # regression_tankYear.py
    scor.to_csv(baseDir + "raw/"+custD+"/scor_smooth.csv",index=False)

fist  = mist[[idField,'day','ref','act','act_raw']].groupby([idField,'day']).agg(sum).reset_index()
fist.loc[:,"corr_f"] = fist['act']/dist['act']
mist.loc[:,"corr_f"] = pd.merge(mist,fist,on=["day",idField],how="left")['corr_f']
mist.loc[:,"act"] = mist['act']*mist['corr_f']
del mist['corr_f']
fist.loc[:,"corr_f"] = fist['ref']/dist['ref']
mist.loc[:,"corr_f"] = pd.merge(mist,fist,on=["day",idField],how="left")['corr_f']
mist.loc[:,"ref"] = mist['ref']*mist['corr_f']
poiId = poi[['id_poi',idField]]
mist = pd.merge(mist,poiId,on=[idField],how="left",suffixes=["_x",""])
...
if ops['remap']: #correct weights
    ...
    corrF = corrF[[idField,'corr_reg','corr_ref']].groupby([idField]).agg(np.nanmean).reset_index()
    corrF.loc[:,"zone"] = corrF[idField].apply(lambda x: x.split("-")[0])
    map10 = e_m.map2df(json.load(open(baseDir + "/raw/"+custD+"/tank_und_rast_"+dTh+".json")))
    ...
    e_m.writeJson(e_m.df2Map(map10),baseDir + "/raw/"+custD+"/tank_"+dTh+"_reweighted.json")
    #map10.loc[:,"zone"] = map10[['zone','id_poi']].apply(lambda x: str(x[0]) + "-" + str(int(x[1])),axis=1)

mist[['time','id_poi','act','ref']].to_csv(baseDir + "raw/"+custD+"/act_vist_final.csv.gz",index=False,compression="gzip")

#mist = pd.read_csv(baseDir + "raw/"+custD+"/act_vist_final.csv.gz",compression="gzip")
#mist[['day','hour',idField,'act','ref']].to_csv(baseDir + "raw/"+custD+"/act_vist_final.csv",index=False)
...
if ops['boxplot']:
    ...
    print(scorHL[['b_h',idField]].groupby("b_h").agg(len).reset_index())
    print(scorHL[['b_s',idField]].groupby("b_s").agg(len).reset_index())
...
if ops['month_val']:
    poi = pd.read_csv(baseDir + "raw/"+custD+"/poi.csv")
    mist = pd.read_csv(baseDir + "raw/"+custD+"/visit_march_melted.csv.gz",compression="gzip")
    mist.loc[:,"day"] = mist['time'].apply(lambda x:x[0:10])
    mist.loc[:,"hour"] = mist['time'].apply(lambda x:x[11:13])
    distW = pd.read_csv(baseDir + "raw/"+custD+"/weekday_correction.csv")
    mist = pd.merge(mist,distW,on=idField,how="left",suffixes=["","_y"])
    dist  = mist[ [idField,'day','ref','bon','dist_raw']].groupby([idField,'day']).agg(sum).reset_index()
    corMat = distW[[idField,'bast','bast_we','bast_su','bast_fr','fact_sun','fact_fri','dir_count']].cor()
    ...
    gist = mist[[idField,'wday','y_dif']].groupby([idField,'wday']).apply(clampF).reset_index()
    Z = linkage(poi[['x','y']], 'ward')
    gradMeter = 111122.19769899677
    distW = pd.read_csv(baseDir + "raw/"+custD+"/weekday_correction.csv")
    distW = pd.merge(distW,poi[[idField,'chirality','pop_dens','submask','type']],on=idField,how="left")
