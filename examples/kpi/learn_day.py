...
ical = ical.rename(columns={x:y for x,y in zip(dateL['isocal'],dateL['day'])})
ical = ical[ical[idField] == ical[idField]]
ical.loc[:,idField] = ical[idField].apply(lambda x: str(int(float(x))))
mist = mist[mist[idField].isin(poi.loc[poi['use'] == 3,idField])]
ical = ical[ical[idField].isin(poi.loc[poi['use'] == 3,idField])]
gact = pd.read_csv(baseDir + "raw/"+custD+"/act_weighted_"+version+".csv.gz",compression="gzip",dtype={idField:str})
...
# gact[idField] = gact[idField].apply(lambda x: str(int(float(x))))
# mist[idField] = mist[idField].apply(lambda x: str(int(float(x))))
# ical[idField] = ical[idField].apply(lambda x: str(int(float(x))))
# dirc[idField] = dirc[idField].apply(lambda x: str(int(float(x))))
# print( "type gact %s mist %s dirc %s" % (gact[idField].dtypes,mist[idField].dtypes,dirc[idField].dtypes))
...
if False:
    ...
    plt.show()
    shl.plotHistogram(tist['humidity'],label="correlation")
    plt.show()
    
scorM = t_s.scorPerf(tist,step="cilac",idField=idField)
...
importlib.reload(t_e)
vist, xist, kpiL = t_e.xvalRegressor(tist,idField,hL,sL)
...
scorR.to_csv(baseDir+"raw/"+custD+"/act_ref_19_03_scor.csv",index=False)
